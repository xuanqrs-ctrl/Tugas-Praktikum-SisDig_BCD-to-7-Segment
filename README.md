...
* **Lamp Test ($\overline{	ext{LT}}$ - Pin 3):** Aktif LOW. Harus dihubungkan ke VCC (HIGH) untuk operasi normal. Jika diberi LOW, semua segmen display dipaksa menyala untuk pengujian LED.
* **Blanking Input ($\overline{	ext{BI}}$ - Pin 4):** Aktif LOW. Harus dihubungkan ke VCC (HIGH) untuk operasi normal. Jika diberi LOW, display akan mati total (*blank*).
* **Latch Enable (LE - Pin 5):** Aktif HIGH. Dihubungkan ke GND (LOW) agar data input langsung diteruskan ke output tanpa dikunci (*latch*).
